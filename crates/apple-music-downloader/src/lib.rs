...
use apple_music_decryptor::{NativePlatform, SessionRuntime, install_android_log_shim};
...
async fn run_server(config: AppConfig) -> AppResult<()> {
    ...
    let platform = Arc::new(NativePlatform::bootstrap(config.native_platform_config())?);
    ...
    tokio::spawn(run_daemon_server(config, platform, state))
        .await
        .map_err(|error| error::AppError::Message(format!("daemon server panicked: {error}")))??;
    Ok(())
}
